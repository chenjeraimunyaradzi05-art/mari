...
```
Browser → Netlify (Next.js) → Backend API (Express)
...
2. Authenticated requests:
   ...
   → Netlify middleware rewrites to backend API (edge function)
   → Authorization: Bearer <accessToken> header forwarded

3. Token refresh:
   Browser POST /api/auth/refresh (with HttpOnly cookie)
   → Next.js API route handler forwards cookie to backend API
...
**Current:** PostgreSQL on Neon (auto-managed `DATABASE_URL`)
