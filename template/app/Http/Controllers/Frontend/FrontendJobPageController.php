...
final class FrontendJobPageController extends Controller
{
    function index(Request $request) : View {
        ...
        $jobs->each(function (Job $job) use ($today): void {
            $locationCountry = $job->company?->companyCountry?->name ?? $job->country?->name;
            $locationState = $job->company?->companyState?->name ?? $job->state?->name;
            $locationCity = $job->company?->companyCity?->name ?? $job->city?->name;

            if (filled($locationCountry) || filled($locationState) || filled($locationCity)) {
                return;
            }

            Log::notice('Job listing missing location metadata', [
                'job_id' => $job->id,
                'job_title' => $job->title,
            ...
            $alreadyLoggedToday = AnalyticsEvent::query()
                ->where('event', 'jobs.location_missing')
                ->where('properties->job_id', $job->id)
                ->whereDate('created_at', $today)
                ->exists();

            if (! $alreadyLoggedToday) {
                AnalyticsEvent::create([
                    'event' => 'jobs.location_missing',
                    'properties' => [
                        'job_id' => $job->id,
                        'job_title' => $job->title,
                        'company_id' => $job->company?->id,
