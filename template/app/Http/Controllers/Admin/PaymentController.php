...
{
    /** OSKO PayID Payment */
    public function oskoPayid() : View {
        abort_if(!$this->checkSession(), 404);
        return view('frontend.pages.osko-payid');
    }

    public function oskoPayidUpload(Request $request): \Illuminate\Http\RedirectResponse {
        abort_if(!$this->checkSession(), 404);
        $request->validate([
        ...
        // Store order as pending, with receipt path
        \App\Services\OrderService::storeOrder(
            uniqid('osko_'),
            'osko-payid',
            $plan['price'],
            config('gatewaySettings.osko_currency_name', 'AUD'),
            'pending',
            [
        ...
        // Optionally, notify admin for manual verification

        \App\Services\OrderService::setUserPlan();
        session()->forget('selected_plan');
        return redirect()->route('company.payment.success')->with('success', 'Receipt uploaded. Your payment will be verified soon.');
    }
     ...
     *
     * @psalm-return array{mode: mixed, sandbox: array{client_id: mixed, client_secret: mixed, app_id: 'APP-80W284485P519543T'}, live: array{client_id: mixed, client_secret: mixed, app_id: mixed}, payment_action: 'Sale', currency: mixed, notify_url: '', locale: 'en_US', validate_ssl: mixed}
     */
    ...
    function paypalSuccess(Request $request): \Illuminate\Http\RedirectResponse
    {
        abort_if(!$this->checkSession(), 404);
    ...
    function paypalCancel(): \Illuminate\Http\RedirectResponse
    {
        return redirect()->route('company.payment.error')->withErrors(['error' => 'Something went wrong please try again']);
    }

    // Pay with Stripe
    function payWithStripe(): \Illuminate\Http\RedirectResponse {
        abort_if(!$this->checkSession(), 404);
