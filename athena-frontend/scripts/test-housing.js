...
async function run() {
  ...
  const res = await fetch(`${base}/api/housing`, { headers: { Authorization: `Bearer ${token}` } })
  const j = await res.json()
  if (!j.properties || !j.properties.length) { console.error('No properties', j); if (child) child.kill(); process.exit(2) }
  console.log('Properties:', j.properties.length)
