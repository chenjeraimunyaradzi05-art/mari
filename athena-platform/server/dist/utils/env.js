...
const hasSearchParam = (value, param, expected) => {
    try {
...
const ENV_VALIDATIONS = [
    ...
    },
    {
        name: 'DATABASE_URL',
        required: true,
        validator: database_url_1.isPostgresConnectionString,
...
function validateEnvironment() {
    ...
    const errors = [];
    const warnings = [];
    const directDatabaseUrlWasDerived = databaseUrls.directDatabaseUrlWasDerived || process.env.ATHENA_DIRECT_DATABASE_URL_DERIVED === 'true';
    if (directDatabaseUrlWasDerived && databaseUrls.databaseUrl && (0, database_url_1.isNeonConnectionString)(databaseUrls.databaseUrl)) {
        warnings.push('DIRECT_DATABASE_URL was derived from DATABASE_URL. Set an explicit unpooled Neon DIRECT_DATABASE_URL in production.');
    }
    ...
    if (databaseUrl && (0, database_url_1.isNeonConnectionString)(databaseUrl)) {
        ...
        if (directUrl && (0, database_url_1.isNeonConnectionString)(directUrl)) {
...
function validateEnvironmentOrExit() {
    ...
    for (const warning of result.warnings) {
        logger_1.logger.warn('Environment warning', { warning });
    }
    if (!result.valid) {
        ...
        logger_1.logger.warn('Server starting with invalid non-production configuration');
    }
    else {
