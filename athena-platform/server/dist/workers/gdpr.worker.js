...
async function processExportRequests() {
    logger_1.logger.info('[GDPR] Starting export job');
    ...
    for (const request of pendingRequests) {
        try {
            ...
            logger_1.logger.error('[GDPR] Export request failed', { error, requestId: request.id });
            failed++;
    ...
    logger_1.logger.info(`[GDPR] Export job completed: ${processed} processed, ${failed} failed`);
...
async function processExportRequest(requestId) {
    ...
    logger_1.logger.info(`[GDPR] Export completed for request ${requestId}`);
}
/**
...
async function processDeletionRequests() {
    logger_1.logger.info('[GDPR] Starting deletion job');
    const pendingRequests = await prisma_1.prisma.dSARRequest.findMany({
        where: {
    ...
    for (const request of pendingRequests) {
        try {
            ...
            logger_1.logger.error('[GDPR] Deletion request failed', { error, requestId: request.id });
            failed++;
        }
    }
    logger_1.logger.info(`[GDPR] Deletion job completed: ${processed} processed, ${failed} failed`);
    return { processed, failed };
}
/**
...
async function processDeletionRequest(requestId) {
    ...
    logger_1.logger.info(`[GDPR] Deletion completed for request ${requestId}`);
}
// ==========================================
// CLEANUP JOBS
// ==========================================
...
async function cleanupExpiredExports() {
    logger_1.logger.info('[GDPR] Starting export cleanup');
    ...
    logger_1.logger.info(`[GDPR] Cleaned up ${expiredRequests.length} expired exports`);
