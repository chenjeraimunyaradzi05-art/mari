...
// ===========================================
router.post('/me/woman-verification', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (!user.womanSelfAttested) {
            throw new errorHandler_1.ApiError(403, 'Women-only verification requires self-attestation');
        }
        if (!user.subscription || user.subscription.status !== 'ACTIVE' || user.subscription.tier === 'FREE') {
            throw new errorHandler_1.ApiError(402, 'Paid subscription required for women verification');
        }
...
const syncUserToIndex = async (userId) => {
    try {
        ...
        logger_1.logger.error(`Failed to sync user ${userId} to OpenSearch`, { error });
    }
};
// ===========================================
