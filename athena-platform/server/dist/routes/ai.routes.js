...
function normalizeOpportunity(job) {
    const skills = (job.skills || []).map((jobSkill) => jobSkill.skill?.name).filter(Boolean);
...
async function opportunityRadarHandler(req, res, next) {
    try {
...
router.get('/opportunity-radar', auth_1.authenticate, auth_1.requirePremium, opportunityRadarHandler);
router.post('/opportunity-radar', auth_1.authenticate, auth_1.requirePremium, opportunityRadarHandler);
// ===========================================
// RESUME OPTIMIZER
// ===========================================
...
router.post('/interview-coach/feedback', auth_1.authenticate, auth_1.requirePremium, async (req, res, next) => {
    try {
        const { question, answer, jobRole, interviewType, difficulty } = req.body;
        if (!question || !answer) {
            throw new errorHandler_1.ApiError(400, 'Question and answer are required');
        }
        const data = await ai_service_1.aiService.evaluateInterviewAnswer({
            question,
            answer,
            jobRole,
            interviewType,
