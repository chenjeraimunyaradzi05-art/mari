...
router.post('/', auth_1.authenticate, (0, auth_1.requireRole)('EMPLOYER', 'EDUCATION_PROVIDER', 'ADMIN'), [
    (0, express_validator_1.body)('title').isString().notEmpty().isLength({ max: 200 }).withMessage('Title max 200 characters'),
    (0, express_validator_1.body)('description').isString().notEmpty().isLength({ max: 10000 }).withMessage('Description max 10000 characters'),
    (0, express_validator_1.body)('framework').isString().notEmpty().isLength({ max: 100 }).withMessage('Framework max 100 characters'),
