...
router.post('/gifts/send', auth_1.authenticate, [
    ...
    (0, express_validator_1.body)('giftType').isString().notEmpty().isLength({ max: 50 }).withMessage('Gift type required'),
 ...
 */
router.post('/balance/purchase/confirm', auth_1.authenticate, [(0, express_validator_1.body)('paymentIntentId').isString().notEmpty().isLength({ max: 256 }).withMessage('paymentIntentId is required')], async (req, res, next) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            throw new errorHandler_1.ApiError(400, 'Validation failed: ' + errors.array().map(e => e.msg).join(', '));
