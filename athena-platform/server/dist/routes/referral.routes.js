 ...
 */
router.post('/track', auth_1.authenticate, async (req, res, next) => {
    try {
 ...
 */
router.post('/:id/complete', auth_1.authenticate, async (req, res, next) => {
    try {
        const { id } = req.params;
 ...
 */
router.get('/share-links', auth_1.authenticate, async (req, res, next) => {
    try {
        const userId = req.user.id;
