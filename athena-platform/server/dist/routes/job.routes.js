    ...
    try {
        ...
        if (search) {
            try {
                ...
                logger_1.logger.error('Search service failed', { error });
            }
        }
...
router.post('/', auth_1.authenticate, (0, auth_1.requireRole)('EMPLOYER', 'ADMIN'), [
    ...
    try {
        ...
        if (skills && skills.length > 0) {
            ...
            const existingSkills = await prisma_1.prisma.skill.findMany({
            ...
            const allSkills = await prisma_1.prisma.skill.findMany({
                where: { name: { in: normalizedSkills } },
            });
