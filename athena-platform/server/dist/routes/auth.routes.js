...
router.post('/register', [
    ...
    try {
        ...
        (0, email_1.sendVerificationEmail)(email, firstName, verificationToken).catch(console.error);
        // Generate tokens
        const tokenPayload = {
            userId: user.id,
            email: user.email,
...
router.post('/login', [
    ...
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            throw new errorHandler_1.ApiError(400, 'Invalid email or password');
        }
...
// FORGOT PASSWORD
// ===========================================
router.post('/forgot-password', [(0, express_validator_1.body)('email').isEmail().normalizeEmail()], async (req, res, next) => {
    try {
        ...
        if (user) {
            ...
            await (0, email_1.sendPasswordResetEmail)(email, user.firstName, resetToken);
        }
        res.json({
            success: true,
...
router.get('/verify-email', async (req, res, next) => {
    try {
        ...
        if (!token || typeof token !== 'string') {
            throw new errorHandler_1.ApiError(400, 'Verification token required');
        }
        // Find valid token
        const verificationToken = await prisma_1.prisma.verificationToken.findFirst({
            where: {
        ...
        // Send welcome email
        await (0, email_1.sendWelcomeEmail)(verificationToken.user.email, verificationToken.user.firstName);
        res.json({
            success: true,
...
router.post('/resend-verification', auth_1.authenticate, async (req, res, next) => {
    try {
        ...
        if (user.emailVerified) {
            throw new errorHandler_1.ApiError(400, 'Email already verified');
        }
