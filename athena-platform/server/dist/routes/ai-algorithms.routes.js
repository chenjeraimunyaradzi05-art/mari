...
router.get('/career-compass', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const prediction = await prisma_1.prisma.careerPrediction.findFirst({
            where: {
                userId,
...
router.post('/career-compass/generate', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        if (isProductionRuntime && !allowPlaceholderAlgorithms) {
            throw new errorHandler_1.ApiError(503, 'Career Compass generation is not configured for production. Connect the ML service or set AI_ALGORITHMS_ALLOW_PLACEHOLDER=true only for verification environments.');
        }
        // Development placeholder until the ML-backed generator is wired to this route.
        const prediction = await prisma_1.prisma.careerPrediction.create({
            data: {
                userId,
                predictedRoles: [
...
router.get('/opportunity-scan', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const opportunities = await prisma_1.prisma.opportunityMatch.findMany({
            where: {
                userId,
...
router.patch('/opportunity-scan/:id/view', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const { id } = req.params;
        const opportunity = await prisma_1.prisma.opportunityMatch.update({
            where: { id, userId },
            data: {
                isViewed: true,
                viewedAt: new Date(),
...
router.patch('/opportunity-scan/:id/feedback', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
...
router.post('/salary-equity/submit', async (req, res, next) => {
    try {
        ...
        const dataPoint = await prisma_1.prisma.salaryDataPoint.create({
            data: {
                userId,
                jobTitle,
...
router.get('/salary-equity/analyze', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const salaryData = await prisma_1.prisma.salaryDataPoint.findMany({
            where: {
        ...
        const analysis = await prisma_1.prisma.salaryAnalysis.create({
            data: {
                userId,
                targetRole: role,
...
router.get('/salary-equity/my-analyses', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const analyses = await prisma_1.prisma.salaryAnalysis.findMany({
            where: { userId },
            orderBy: { generatedAt: 'desc' },
            take: 10,
        });
...
router.get('/mentor-match', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const matches = await prisma_1.prisma.mentorMatchScore.findMany({
            where: {
                menteeId: userId,
                isActive: true,
            },
...
router.get('/mentor-match/:mentorId', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const match = await prisma_1.prisma.mentorMatchScore.findUnique({
            where: {
                menteeId_mentorId: {
                    menteeId: userId,
                    mentorId,
...
router.get('/trust-score', async (req, res, next) => {
    try {
        ...
        let trustScore = await prisma_1.prisma.userTrustScore.findUnique({
            where: { userId },
        });
        // Create default trust score if doesn't exist
        if (!trustScore) {
            trustScore = await prisma_1.prisma.userTrustScore.create({
                data: {
                    userId,
                    trustScore: 50,
                    badges: [],
...
router.get('/trust-score/:userId', async (req, res, next) => {
    try {
        const { userId } = req.params;
        const trustScore = await prisma_1.prisma.userTrustScore.findUnique({
            where: { userId },
            select: {
                trustScore: true,
                badges: true,
...
router.post('/report', async (req, res, next) => {
    try {
        ...
        const report = await prisma_1.prisma.contentReport.create({
            data: {
                reporterId: userId,
                contentType,
                contentId,
...
router.get('/creator-analytics', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        let analytics = await prisma_1.prisma.creatorAnalytics.findUnique({
            where: { userId },
        });
        // Create default analytics if doesn't exist
        if (!analytics) {
            analytics = await prisma_1.prisma.creatorAnalytics.create({
                data: {
                    userId,
                    creatorTier: 'BRONZE',
                },
...
router.get('/creator-analytics/projections', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const analytics = await prisma_1.prisma.creatorAnalytics.findUnique({
            where: { userId },
            select: {
                followerCount: true,
        ...
        if (!analytics.projectedIncome) {
            ...
            await prisma_1.prisma.creatorAnalytics.update({
                where: { userId },
                data: {
...
router.get('/feed-preferences', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        let prefs = await prisma_1.prisma.userFeedPreferences.findUnique({
            where: { userId },
        });
        // Create default preferences if doesn't exist
        if (!prefs) {
            prefs = await prisma_1.prisma.userFeedPreferences.create({
                data: {
                    userId,
                    followedCategories: [],
                    followedHashtags: [],
...
router.patch('/feed-preferences', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const prefs = await prisma_1.prisma.userFeedPreferences.upsert({
            where: { userId },
            update: {
...
router.post('/feed-preferences/search', async (req, res, next) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        const prefs = await prisma_1.prisma.userFeedPreferences.findUnique({
            where: { userId },
        });
        ...
        await prisma_1.prisma.userFeedPreferences.upsert({
            where: { userId },
