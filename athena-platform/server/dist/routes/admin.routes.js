...
const generateInviteCode = (prefix) => {
    const base = crypto_1.default.randomBytes(5).toString('hex').toUpperCase();
    ...
    try {
        ...
        if (isSuspended !== undefined) {
            ...
            if (isSuspended && suspensionReason) {
                ...
                logger_1.logger.info('User suspended', { userId: id, reason: suspensionReason, adminId: req.user?.id });
            }
        }
...
router.patch('/content/posts/:id', async (req, res, next) => {
    try {
        ...
        logger_1.logger.info('Admin moderation action', { postId: id, action, reason: reason || 'None provided', adminId: req.user?.id });
        res.json(result);
    }
    catch (error) {
        next(error);
...
router.get('/invite-codes', async (req, res, next) => {
    try {
        ...
        const [inviteCodes, total] = await Promise.all([
            ...
            prisma_1.prisma.inviteCode.count({ where }),
        ]);
        res.json({
            inviteCodes,
            pagination: {
...
router.post('/invite-codes', async (req, res, next) => {
    try {
        ...
        const created = await prisma_1.prisma.inviteCode.createMany({ data: codes, skipDuplicates: true });
        const createdRecords = await prisma_1.prisma.inviteCode.findMany({
 ...
 */
router.patch('/invite-codes/:id', async (req, res, next) => {
    try {
        const { id } = req.params;
        ...
        const inviteCode = await prisma_1.prisma.inviteCode.update({
            where: { id },
            data: { isActive },
        });
        await (0, audit_1.logAudit)({
 ...
 */
router.get('/woman-verifications', async (req, res, next) => {
    try {
 ...
 */
router.patch('/woman-verifications/:userId', async (req, res, next) => {
    try {
