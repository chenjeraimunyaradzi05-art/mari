...
class DataRetentionService {
    ...
    async runAllPurgeJobs() {
        ...
        const results = [];
        const errors = [];
        logger_1.logger.info('[DataRetention] Starting purge jobs...');
        // Check for legal holds first
        const activeHolds = await prisma_1.prisma.legalHold.findMany({
            where: { isActive: true },
        });
        ...
        for (const job of jobs) {
            try {
                ...
                logger_1.logger.error('[DataRetention] Job failed', { error });
            }
        }
        ...
        logger_1.logger.info(`[DataRetention] Completed. Purged ${totalPurged} records.`);
        return {
            startedAt,
            completedAt,
            results,
    ...
    async purgeExpiredVerificationTokens() {
        const result = await prisma_1.prisma.verificationToken.deleteMany({
            where: {
    ...
    async purgeOldMessages(excludeUserIds, isHeld) {
        ...
        const result = await prisma_1.prisma.message.deleteMany({ where: whereClause });
        return {
            dataType: 'messages',
            recordsPurged: result.count,
            errors: [],
    ...
    async purgeSoftDeletedUsers(excludeUserIds) {
        ...
        const pendingDeletions = await prisma_1.prisma.dSARRequest.findMany({
            where: {
                type: 'DELETION',
                status: 'COMPLETED',
    ...
    async purgeExpiredDSARExports() {
        const result = await prisma_1.prisma.dSARRequest.updateMany({
            where: {
                type: 'EXPORT',
                status: 'COMPLETED',
    ...
    async anonymizeOldAuditLogs() {
        ...
        const result = await prisma_1.prisma.auditLog.updateMany({
            where: {
                createdAt: { lt: cutoffDate },
                metadata: {
                    path: ['anonymized'],
    ...
    async getRetentionPolicies() {
        return prisma_1.prisma.retentionPolicy.findMany({
    ...
    async initializeRetentionPolicies() {
        ...
        for (const policy of policies) {
            await prisma_1.prisma.retentionPolicy.upsert({
...
if (require.main === module) {
    exports.dataRetentionService.runAllPurgeJobs()
        .then((summary) => {
        logger_1.logger.info('Purge job completed', { summary });
        process.exit(0);
    })
        .catch((error) => {
        logger_1.logger.error('Purge job failed', { error });
        process.exit(1);
    });
}
