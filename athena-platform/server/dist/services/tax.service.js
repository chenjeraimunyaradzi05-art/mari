...
async function updateTaxReturn(id, userId, data) {
    ...
    if (record.userId !== userId) {
        throw new errorHandler_1.ApiError(403, 'Not authorized to update this tax return');
    }
...
async function submitTaxReturn(id, userId) {
    const record = await prisma_1.prisma.taxReturn.findUnique({ where: { id } });
    if (!record)
        throw new errorHandler_1.ApiError(404, 'Tax return not found');
    // Verify ownership
    if (record.userId !== userId) {
        throw new errorHandler_1.ApiError(403, 'Not authorized to submit this tax return');
    }
    if (record.status !== 'DRAFT') {
...
async function deleteTaxReturn(id, userId) {
    const record = await prisma_1.prisma.taxReturn.findUnique({ where: { id } });
    if (!record)
        throw new errorHandler_1.ApiError(404, 'Tax return not found');
    // Verify ownership
    if (record.userId !== userId) {
        throw new errorHandler_1.ApiError(403, 'Not authorized to delete this tax return');
    }
    if (record.status !== 'DRAFT') {
