...
exports.chat = chat;
exports.getProactiveSuggestions = getProactiveSuggestions;
 ...
 */
async function handleIntent(intent, params, userId) {
    switch (intent) {
        case 'find_jobs':
            return {
...
async function getUserContext(userId) {
    try {
        ...
        logger_1.logger.error('Error getting user context:', error);
        return {
            name: 'there',
...
function buildSystemPrompt(userContext, context) {
...
function generateActions(message, userContext) {
    const actions = [];
    const lowerMessage = message.toLowerCase();
...
function generateSuggestions(userContext, context) {
    const suggestions = [];
    if (userContext.profileCompleteness < 80) {
...
function getSimulatedResponse(message, userContext) {
    const lowerMessage = message.toLowerCase();
 ...
 */
async function getOnboardingSteps(userId) {
    try {
        ...
        logger_1.logger.error('Failed to get onboarding steps', { error, userId });
        return [];
    }
}
