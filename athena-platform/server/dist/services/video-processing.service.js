...
function canUseSimulatedVideoProcessing() {
    return !isProductionRuntime || process.env.VIDEO_ALLOW_SIMULATION === 'true';
}
function requireVideoProcessingRuntime(feature) {
    if (canUseSimulatedVideoProcessing() || VIDEO_PROCESSOR_URL)
        return;
    throw new errorHandler_1.ApiError(503, `Video ${feature} is not configured for production. Set VIDEO_PROCESSOR_URL for the media processor, or set VIDEO_ALLOW_SIMULATION=true only for non-production verification.`);
}
async function callVideoProcessor(path, payload) {
    if (!VIDEO_PROCESSOR_URL) {
        throw new errorHandler_1.ApiError(503, 'VIDEO_PROCESSOR_URL is required for production video processing');
    }
    ...
    if (!response.ok) {
        ...
        throw new errorHandler_1.ApiError(502, `Video processor request failed (${response.status})${body ? `: ${body.slice(0, 300)}` : ''}`);
    }
...
async function generateThumbnail(videoUrl, timestamp = 0) {
    ...
    if (!canUseSimulatedVideoProcessing()) {
        const result = await callVideoProcessor('/thumbnail', {
            videoUrl,
            timestamp,
        });
...
async function applyEffects(videoUrl, effects) {
    ...
    if (!canUseSimulatedVideoProcessing()) {
        const result = await callVideoProcessor('/effects', {
            videoUrl,
            effects,
        });
