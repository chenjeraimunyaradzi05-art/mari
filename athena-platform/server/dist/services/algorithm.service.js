...
exports.getCareerCompass = getCareerCompass;
exports.getOpportunityScan = getOpportunityScan;
exports.getSalaryEquity = getSalaryEquity;
exports.getMentorMatch = getMentorMatch;
exports.getIncomeStream = getIncomeStream;
exports.getRecommendationEngineV2 = getRecommendationEngineV2;
...
async function getCareerCompass(userId, targetRole) {
    ...
    const requiredSkills = uniq(jobs.flatMap((job) => job.skills.map((skill) => normalizeSkill(skill.skill.name))));
    const missingSkills = requiredSkills.filter((skill) => !userSkills.includes(skill));
    const recommendedCourses = missingSkills.length
        ? await prisma_1.prisma.course.findMany({
            where: {
                isActive: true,
...
async function getOpportunityScan(userId) {
    const now = new Date();
...
async function getSalaryEquity(userId, targetRole) {
    const user = await prisma_1.prisma.user.findUnique({
        where: { id: userId },
        select: {
...
async function getMentorMatch(userId) {
    const userSkills = await getUserSkills(userId);
...
async function getIncomeStream(userId) {
    ...
    const [creatorProfile, giftStats, recentPosts] = await Promise.all([
        ...
        prisma_1.prisma.giftTransaction.aggregate({
    ...
    const diversificationScore = Math.max(10, Math.min(100, 40 + (recentPosts > 6 ? 20 : 0) + (monthlyEarnings > 200 ? 20 : 0)));
    const creatorStatus = creatorProfile?.isMonetized
        ? followerCount > 20000
            ? 'established'
            : followerCount > 3000
                ? 'growing'
...
async function getRecommendationEngineV2(userId) {
    const [user, userSkills] = userId
        ? await Promise.all([
            prisma_1.prisma.user.findUnique({ where: { id: userId }, select: { persona: true } }),
