...
class BreachNotificationService {
    ...
    async reportBreach(report) {
        const breach = await prisma_1.prisma.dataBreach.create({
            data: {
                title: report.title,
    ...
    async updateBreachStatus(breachId, updates) {
        ...
        const breach = await prisma_1.prisma.dataBreach.update({
            where: { id: breachId },
            data: updateData,
        });
        await prisma_1.prisma.privacyAuditLog.create({
    ...
    async notifyRegulator(notification) {
        const breach = await prisma_1.prisma.dataBreach.findUnique({
      ...
      `,
        ...
        const updatedBreach = await prisma_1.prisma.dataBreach.update({
            where: { id: notification.breachId },
            data: {
    ...
    async notifyAffectedUsers(breachId, userIds, notificationContent) {
        ...
        // Update breach record
        await prisma_1.prisma.dataBreach.update({
            where: { id: breachId },
            data: {
    ...
    async getBreachesRequiringNotification() {
        ...
        return prisma_1.prisma.dataBreach.findMany({
            where: {
    ...
    async generateBreachReport(breachId) {
        ...
        const auditLogs = await prisma_1.prisma.privacyAuditLog.findMany({
            where: {
                resourceType: 'DataBreach',
                resourceId: breachId,
            },
