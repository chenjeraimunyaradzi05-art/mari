 ...
 */
async function verifyItemAccess(itemId, userId) {
    const item = await prisma_1.prisma.inventoryItem.findUnique({
        where: { id: itemId },
        select: { organizationId: true },
    });
    if (!item) {
        throw new errorHandler_1.ApiError(404, 'Item not found');
    }
...
async function verifyLocationAccess(locationId, userId) {
    const location = await prisma_1.prisma.inventoryLocation.findUnique({
        where: { id: locationId },
        select: { organizationId: true },
    });
    if (!location) {
        throw new errorHandler_1.ApiError(404, 'Location not found');
    }
 ...
 */
async function verifyTransactionAccess(transactionId, userId) {
    const transaction = await prisma_1.prisma.inventoryTransaction.findUnique({
        where: { id: transactionId },
        select: { item: { select: { organizationId: true } } },
    });
    if (!transaction) {
        throw new errorHandler_1.ApiError(404, 'Transaction not found');
    }
