...
async function updateMoneyTransaction(id, userId, data) {
    if (data.status && !STATUSES.includes(data.status)) {
        throw new errorHandler_1.ApiError(400, 'Invalid transaction status');
    }
    // Verify ownership before updating
    const transaction = await prisma_1.prisma.moneyTransaction.findUnique({ where: { id } });
    if (!transaction) {
        throw new errorHandler_1.ApiError(404, 'Transaction not found');
    }
    if (transaction.userId !== userId) {
        throw new errorHandler_1.ApiError(403, 'Not authorized to update this transaction');
    }
...
async function deleteMoneyTransaction(id, userId) {
    ...
    if (transaction.userId !== userId) {
        throw new errorHandler_1.ApiError(403, 'Not authorized to delete this transaction');
    }
