...
class AiService {
    ...
    constructor() {
        ...
        if (apiKey) {
            ...
            logger_1.logger.warn('AI_OPENAI_API_KEY not found. AI features will be simulated when allowed.');
            if (this.isProduction && !this.allowSimulation) {
                logger_1.logger.error('AI service not configured in production. Set AI_OPENAI_API_KEY or OPENAI_API_KEY.');
            }
        }
    }
    shouldSimulate() {
    ...
    async evaluateInterviewAnswer(params) {
        if (!this.openai) {
            ...
            return {
                feedback: 'Development simulation: connect AI_OPENAI_API_KEY or OPENAI_API_KEY for production interview feedback.',
                analysis: {
                    rating: 3,
...
Candidate answer: ${params.answer}`;
            ...
            logger_1.logger.error('AI Interview Answer Feedback failed:', error);
            throw error;
        }
    }
