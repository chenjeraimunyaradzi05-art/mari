...
class NotificationService {
    ...
    async notify(options) {
        ...
        const resolvedMessage = message || (i18nKey ? i18n_service_1.i18nService.tSync(i18nKey, i18nParams, locale) : undefined);
        const promises = [];
        ...
        if (channels.includes('email') && user.email) {
            if (this.shouldSend(user.notificationPreferences, 'email', type)) {
                promises.push(this.sendEmailSafely(user.email, {
                    ...options,
                    message: resolvedMessage,
                }));
            }
        ...
        if (process.env.FIREBASE_PROJECT_ID) {
            try {
                ...
                try {
                    ...
                    admin = await Promise.resolve().then(() => __importStar(require('firebase-admin')));
                }
                catch {
                    logger_1.logger.warn('firebase-admin package not installed; push notifications disabled');
                    return;
                }
