...
class ConsentService {
    ...
    async initializeUserConsents(userId, context) {
        ...
        for (const consentType of requiredConsents) {
            await prisma_1.prisma.consentRecord.upsert({
    ...
    async hasConsent(userId, consentType) {
        const consent = await prisma_1.prisma.consentRecord.findUnique({
            where: { userId_consentType: { userId, consentType } },
        });
    ...
    async hasConsents(userId, consentTypes) {
        const consents = await prisma_1.prisma.consentRecord.findMany({
            where: {
                userId,
    ...
    async withdrawAllOptionalConsents(userId, context) {
        const optionalTypes = Object.entries(exports.CONSENT_DESCRIPTIONS)
            .filter(([_, config]) => !config.required)
            .map(([type]) => type);
        await prisma_1.prisma.consentRecord.updateMany({
            where: {
                userId,
    ...
    async getConsentHistory(userId) {
        return prisma_1.prisma.privacyAuditLog.findMany({
            where: {
                userId,
                action: {
