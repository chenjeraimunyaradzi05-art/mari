...
class GDPRService {
    ...
    async getDSARRequests(userId) {
        return prisma_1.prisma.dSARRequest.findMany({
            where: { userId },
    ...
    async getDSARRequest(requestId) {
        return prisma_1.prisma.dSARRequest.findUnique({
            where: { id: requestId },
        });
    }
    /**
    ...
    async createDSARRequest(input) {
        ...
        const dsar = await prisma_1.prisma.dSARRequest.create({
            data: {
                userId: input.userId,
                type: input.type,
    ...
    async processExportRequest(dsarId) {
        const dsar = await prisma_1.prisma.dSARRequest.findUnique({
            where: { id: dsarId },
            include: { user: true },
        });
        if (!dsar)
        ...
        const [user, posts, comments, messages, likes, followers, following, groups, events, jobs, applications, courses, mentorSessions, subscription, auditLogs, consents,] = await Promise.all([
            ...
            // Likes
            prisma_1.prisma.like.findMany({ where: { userId } }),
            // Followers
            prisma_1.prisma.follow.findMany({ where: { followingId: userId } }),
            // Following
            prisma_1.prisma.follow.findMany({ where: { followerId: userId } }),
            // Groups
            ...
            // Events
            prisma_1.prisma.eventRegistration.findMany({
                where: { userId },
                include: { event: true },
            }),
            // Jobs posted
            prisma_1.prisma.job.findMany({ where: { postedById: userId } }),
            // Job applications
            prisma_1.prisma.jobApplication.findMany({ where: { userId } }),
            // Courses
            ...
            // Mentor sessions
            prisma_1.prisma.mentorSession.findMany({ where: { menteeId: userId } }),
            // Subscription
            prisma_1.prisma.subscription.findFirst({ where: { userId } }),
            // Audit logs
            prisma_1.prisma.auditLog.findMany({ where: { actorUserId: userId } }),
            // Consent records
    ...
    async recordConsent(userId, consentType, granted, context) {
        ...
        const consent = await prisma_1.prisma.consentRecord.upsert({
            where: {
    ...
    async getUserConsents(userId) {
        return prisma_1.prisma.consentRecord.findMany({
            where: { userId },
    ...
    async recordCookieConsent(visitorId, preferences, context) {
        ...
        const cookieConsent = await prisma_1.prisma.cookieConsent.upsert({
            where: { visitorId },
            update: {
    ...
    async getCookieConsent(visitorId) {
        return prisma_1.prisma.cookieConsent.findUnique({
            where: { visitorId },
        });
    }
    // ============================================
    ...
    async logPrivacyAction(params) {
        await prisma_1.prisma.privacyAuditLog.create({
            data: {
