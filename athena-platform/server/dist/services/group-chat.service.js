 ...
 * Unmute a member
 */
async function unmuteMember(groupId, actorId, userId) {
    try {
        await enforcePermission(groupId, actorId, 'mute_members');
        await prisma_1.prisma.groupMember.update({
            where: { groupId_userId: { groupId, userId } },
        ...
        logger_1.logger.info('Member unmuted', { groupId, userId });
    }
    catch (error) {
        if (error instanceof errorHandler_1.ApiError)
            throw error;
        logger_1.logger.error('Failed to unmute member', { error, groupId, userId });
        throw error;
    }
}
/**
