...
exports.getMentorProfileById = getMentorProfileById;
...
const stripe = new stripe_1.default(process.env.STRIPE_SECRET_KEY || 'sk_test_not_configured', {
...
async function getMentorProfileById(mentorId) {
    return prisma_1.prisma.mentorProfile.findUnique({
        where: { id: mentorId },
        include: {
            user: {
                select: {
 ...
 */
async function rescheduleSession(sessionId, userId, data) {
    const session = await prisma_1.prisma.mentorSession.findUnique({
        where: { id: sessionId },
        include: { mentorProfile: true },
    });
    ...
    if (!['REQUESTED', 'CONFIRMED'].includes(session.status)) {
        throw new errorHandler_1.ApiError(400, 'Only requested or confirmed sessions can be rescheduled');
    }
    ...
    const nearbySessions = await prisma_1.prisma.mentorSession.findMany({
        where: {
            id: { not: sessionId },
    ...
    const hasConflict = nearbySessions.some((nearbySession) => {
        if (!nearbySession.scheduledAt)
            return false;
        const nearbyStart = nearbySession.scheduledAt;
        const nearbyEnd = new Date(nearbyStart.getTime() + nearbySession.durationMinutes * 60 * 1000);
        return nearbyStart < scheduledEnd && nearbyEnd > scheduledAt;
    });
    if (hasConflict) {
        throw new errorHandler_1.ApiError(409, 'New session time conflicts with an existing booking');
    }
    const updated = await prisma_1.prisma.mentorSession.update({
        where: { id: sessionId },
        data: {
