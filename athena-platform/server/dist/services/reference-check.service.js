...
async function getReferenceByToken(token) {
    ...
    return {
        request: {
            ...
            questions: reference.customQuestions,
...
async function submitReferenceResponse(token, response) {
    ...
    const questions = (reference.customQuestions || []);
    ...
    logger_1.logger.info(`Reference ${reference.id} completed, candidate ${reference.candidateId} will be notified`);
...
async function declineReferenceRequest(token, reason) {
    ...
    await prisma_1.prisma.referenceRequest.update({
        ...
        data: {
            status: 'DECLINED',
            responses: { declined: true, reason },
        },
    });
