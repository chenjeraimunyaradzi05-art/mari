...
export async function initializeIndexes(client: Client) {
    ...
    try {
      ...
      if (!exists.body) {
        ...
        try {
          ...
          logger.info(`Updated mappings for: ${indexName}`);
        } catch (mappingError: any) {
          // Mapping conflicts are expected if fields can't be changed
          logger.warn(`Could not update mappings for ${indexName}: ${mappingError.message}`);
        }
      }
    } catch (error: any) {
      logger.error(`Failed to initialize index ${indexName}`, { error: error.message });
    }
  }

  logger.info('OpenSearch index initialization complete');
}
