 ...
 */
async function loadTranslations(locale: SupportedLocale): Promise<Record<string, string>> {
  // Check cache first
  ...
  try {
    ...
    logger.warn(`Failed to load translations for locale ${locale}, using defaults`);
 ...
 */
export async function preloadTranslations(locales: SupportedLocale[]): Promise<void> {
  await Promise.all(locales.map(loadTranslations));
  logger.info(`Preloaded translations for ${locales.length} locales`);
}
 ...
 */
export function i18nNotification(
