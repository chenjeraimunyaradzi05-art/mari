...
router.get('/partners', async (req: Request, res: Response) => {
  try {
    ...
    if (focusArea) where.focusAreas = { has: focusArea as string };
...
router.get('/disability-friendly-employers', async (req: Request, res: Response) => {
  try {
    ...
    const where: Record<string, unknown> = {};
    if (hasRemote === 'true') where.hasRemoteOptions = true;
    if (hasFlexible === 'true') where.hasFlexibleWork = true;
    if (minRating) where.accessibilityRating = { gte: parseInt(minRating as string) };
