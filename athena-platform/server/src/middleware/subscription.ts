 ...
 */
function tierHasPermission(tier: SubscriptionTier, permission: string): PermissionCheck {
 ...
 */
async function getUsageCount(userId: string, feature: string, period: 'day' | 'month'): Promise<number> {
  const now = new Date();
  let startDate: Date;
 ...
 */
async function logUsage(userId: string, feature: string): Promise<void> {
 ...
 */
export function requireSubscription(requiredPermission: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (tier !== 'FREE' && status !== 'ACTIVE' && status !== 'TRIALING') {
        throw new ApiError(402, 'Subscription expired or inactive', {
      ...
      if (!check.hasPermission) {
        throw new ApiError(403, 'Upgrade required for this feature', {
      ...
      if (check.limit) {
        ...
        if (used >= check.limit) {
          throw new ApiError(429, `${period === 'day' ? 'Daily' : 'Monthly'} limit reached`, {
            code: 'LIMIT_REACHED',
            limit: check.limit,
            used,
            period,
            upgradeRequired: tierHasPermission(
              tier === 'FREE' ? 'PREMIUM_CAREER' : 'PREMIUM_PROFESSIONAL',
              requiredPermission
            ).hasPermission
              ? undefined
              : 'PREMIUM_PROFESSIONAL',
        ...
        (req as any).subscriptionUsage = { limit: check.limit, used, remaining: check.limit - used };
      }
...
export function enforceSubscription() {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const flag = await getFeatureFlagByKey(flagKey);
      
      if (flag) {
        const userId = (req as any).user?.id;
        const isEnabled = evaluateFeatureFlag(flag, userId);
 ...
 * Compare two tiers
 */
export function compareTiers(tier1: SubscriptionTier, tier2: SubscriptionTier): -1 | 0 | 1 {
  const tierOrder: SubscriptionTier[] = [
    'FREE',
    'PREMIUM_CAREER',
    'PREMIUM_PROFESSIONAL',
