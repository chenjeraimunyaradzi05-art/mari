...
export default function PurchasePage() {
  ...
  const [paying, setPaying] = useState<{ clientSecret: string | null; amount: number } | null>(null);
  ...
  const pay = async () => { if (!p) return; setBusy(true); try { const res = await autoApi.pay(p.id); const pay = res.data.data.payment; setPaying({ clientSecret: pay?.clientSecret ?? null, amount: pay?.amount ?? (p.agreedAmount ?? p.offerAmount) * 100 }); } catch (err) { toast.error(autoError(err, 'The payment could not be started.')); } finally { setBusy(false); } };
  const payInspection = async (id: string) => { setBusy(true); try { const res = await autoApi.payInspection(id); if (res.data.data.alreadyHeld) toast('Already held.'); else setPaying({ clientSecret: res.data.data.clientSecret ?? null, amount: res.data.data.amount }); } catch (err) { toast.error(autoError(err, 'The fee could not be paid.')); } finally { setBusy(false); } };
            ...
            <div className="space-y-6">
              <Panel title="Where it is" intro={p.nextStep}>
                <dl className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
                  ...
                  <div><dt className="text-[11px] uppercase tracking-wide text-slate-500">The money</dt><dd className="text-slate-800 dark:text-slate-200">{p.escrow ? (p.escrow.status === 'CAPTURED' ? 'Released to the seller' : ['CANCELED', 'REFUNDED'].includes(p.escrow.status) ? 'Returned to the buyer' : 'Held by ATHENA') : 'Not yet paid'}</dd></div>
            ...
            <div className="space-y-6">
              <Panel title={p.role === 'buyer' ? 'What you can do' : p.role === 'seller' ? 'What you can do' : 'Admin'}>
                <div className="space-y-3">
                  ...
                  {['OFFERED', 'ACCEPTED', 'PAID_HELD'].includes(p.status) && p.role !== 'admin' && <><Field label="Reason, if you cancel"><input value={reason} onChange={(e) => setReason(e.target.value)} maxLength={1000} className={inputClass} /></Field><Confirm label="Cancel this purchase" tone="slate" hint={p.status === 'PAID_HELD' ? 'The held money goes back to the buyer.' : 'Ends this offer.'} onConfirm={() => act(() => autoApi.cancelPurchase(p.id, { reason: reason || undefined }), 'Cancelled')} /></>}
