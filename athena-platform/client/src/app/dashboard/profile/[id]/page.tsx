  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <div className="card overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row md:items-end gap-4 -mt-16 md:-mt-20 relative z-10 px-4">
          <div className="relative">
            {profile.profile?.avatarUrl ? (
              <img
                ...
                className="w-24 h-24 md:w-32 md:h-32 rounded-full object-cover border-4 border-white dark:border-slate-900"
              />
            ) : (
              <div className="w-24 h-24 md:w-32 md:h-32 rounded-full bg-primary-100 dark:bg-primary-900 flex items-center justify-center text-primary-600 font-bold text-3xl border-4 border-white dark:border-slate-900">
                {getInitials(profile.firstName || '', profile.lastName || '')}
              </div>
            )}
            {profile.verified && (
              <div className="absolute bottom-2 right-2 w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-900">
          ...
          <div className="flex-1">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 rounded-full border border-slate-200 dark:border-slate-800 px-3 py-1 text-xs text-slate-600 dark:text-slate-300">
                {typeof safetyScore === 'number' ? (
                  <>
                    <ShieldCheck className="w-4 h-4 text-emerald-500" />
                    Safety Score: <span className="font-semibold text-slate-900 dark:text-white">{safetyScore}</span>
                  </>
                ) : (
                  <>
        ...
        {/* Stats */}
        <div className="flex items-center space-x-6 mt-6 pt-4 border-t border-slate-100 dark:border-slate-800">
              ...
              <div className="space-y-4">
                {profile.posts.slice(0, 3).map((post: any) => (
                  <Link
                    ...
                    className="block p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition"
                  >
