    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6">
          <p className="text-slate-900 dark:text-white font-medium">Unable to load applications</p>
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-10 text-center">
          ...
          <p className="text-slate-900 dark:text-white font-medium">No education applications yet</p>
          <p className="text-slate-600 dark:text-slate-400 text-sm mt-1">Check back later.</p>
        </div>
      ) : (
                ...
                {applications.map((a: any) => (
                  <tr key={a.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          {a.user?.headline ? (
                            <p className="text-sm text-slate-600 dark:text-slate-400 truncate">{a.user.headline}</p>
                          ) : null}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      ...
                      {a.course?.type ? (
                        <p className="text-sm text-slate-600 dark:text-slate-400">{a.course.type}</p>
                      ) : null}
                    </td>
                    <td className="px-6 py-4">
