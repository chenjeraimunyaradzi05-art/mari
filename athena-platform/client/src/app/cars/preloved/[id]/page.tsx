        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[3fr_2fr]">
            <div className="space-y-6">
              ...
              {l.videoUrl && <a href={safeHref(l.videoUrl)} target="_blank" rel="noopener noreferrer" className="inline-block text-sm font-semibold text-rose-600">Watch the seller's walk-around video</a>}
              ...
              <Panel icon={ShieldCheck} title="Before you pay" intro="The checks, in the order that catches the most. None of them takes long.">
                <ul className="space-y-3 text-sm">
                  <li className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800/60"><p className="font-medium text-slate-900 dark:text-white">1. {l.checks.ppsr.name} <span className={cn('ml-1 text-xs', l.checks.ppsr.ready ? 'text-emerald-600' : 'text-amber-600')}>{l.checks.ppsr.ready ? (l.ppsrChecked ? '· the seller says she has one — ATHENA has not checked it, so run your own' : '· VIN available') : '· ask the seller for the VIN first'}</span></p><p className="text-slate-600 dark:text-slate-400">{l.checks.ppsr.what}</p><div className="mt-1 flex flex-wrap gap-3"><a href={safeHref(l.checks.ppsr.url)} target="_blank" rel="noopener noreferrer" className="font-semibold text-rose-600">ppsr.gov.au</a>{l.ppsrCertificateUrl && <a href={safeHref(l.ppsrCertificateUrl)} target="_blank" rel="noopener noreferrer" className="font-semibold text-rose-600">The seller's own certificate (unverified)</a>}</div></li>
                  {l.checks.rego && <li className="rounded-lg bg-slate-50 p-3 dark:bg-slate-800/60"><p className="font-medium text-slate-900 dark:text-white">2. {l.checks.rego.name}</p><p className="text-slate-600 dark:text-slate-400">{l.checks.rego.what}</p><a href={safeHref(l.checks.rego.url)} target="_blank" rel="noopener noreferrer" className="font-semibold text-rose-600">Check the plates</a></li>}
