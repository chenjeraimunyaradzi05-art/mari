      ...
      {d && (
        <div className="space-y-6">
          ...
          <div className="grid gap-6 lg:grid-cols-[3fr_2fr]">
            <div className="space-y-6">
              {d.about && <Panel title="About"><p className="whitespace-pre-line text-sm leading-7 text-slate-800 dark:text-slate-200">{d.about}</p><div className="mt-3 flex flex-wrap gap-3 text-sm">{d.phone && <a href={`tel:${d.phone.replace(/\s+/g, '')}`} className="inline-flex items-center gap-1.5 text-rose-600"><Phone className="h-4 w-4" /> {d.phone}</a>}{d.website && <a href={d.website} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-rose-600"><Globe className="h-4 w-4" /> Website</a>}{d.address && <span className="text-slate-600 dark:text-slate-400">{d.address}</span>}</div>{d.hours && <ul className="mt-3 grid grid-cols-2 gap-1 text-xs text-slate-600 sm:grid-cols-4 dark:text-slate-400">{DAYS.map((day, i) => <li key={day}>{day}: {d.hours?.[String(i)]?.map((r) => `${r[0]}–${r[1]}`).join(', ') ?? 'closed'}</li>)}</ul>}</Panel>}
