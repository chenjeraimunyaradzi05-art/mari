...
type Review = { id: string; rating: number; reliability: number; safetyFeel: number; runningCosts: number; title: string; body: string; ownedMonths: number | null; videoUrl: string | null; isOwner: boolean; isHidden: boolean; helpfulCount: number; by: string; createdAt: string; isYou: boolean };
...
export default function CarPage() {
  ...
  const helpful = async (id: string) => { try { await autoApi.helpful(id); data.reload(); } catch (err) { toast.error(autoError(err, 'That did not work.')); } };
                ...
                <ul className="space-y-4">
                  {c.reviews.map((r) => (
                    <li key={r.id} className={cn('rounded-xl border border-slate-100 p-4 dark:border-slate-800', r.isHidden && 'opacity-60')}>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-3 text-xs">{r.videoUrl && <a href={r.videoUrl} target="_blank" rel="noopener noreferrer" className="font-semibold text-rose-600">Watch her video</a>}{isAuthenticated && !r.isYou && <button type="button" onClick={() => helpful(r.id)} className="text-slate-500 hover:text-rose-600">Helpful ({r.helpfulCount})</button>}{r.isYou && <button type="button" onClick={() => { setForm({ rating: r.rating, reliability: r.reliability, safetyFeel: r.safetyFeel, runningCosts: r.runningCosts, title: r.title, body: r.body, ownedMonths: r.ownedMonths ? String(r.ownedMonths) : '', videoUrl: r.videoUrl ?? '' }); setWriting(true); }} className="text-slate-500 hover:text-rose-600">Edit yours</button>}{c.canModerate && <button type="button" onClick={() => hide(r.id, !r.isHidden)} className="text-slate-500 underline-offset-2 hover:underline">{r.isHidden ? 'Show it' : 'Hide it'}</button>}</div>
                    </li>
                  ))}
                </ul>
              </Panel>
