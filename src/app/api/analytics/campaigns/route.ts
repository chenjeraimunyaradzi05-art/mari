...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      campaigns,
      summary: {
        totalSpend: campaigns.reduce((sum: number, c: CampaignRow) => sum + c.spend, 0),
        totalImpressions: campaigns.reduce((sum: number, c: CampaignRow) => sum + c.impressions, 0),
        totalClicks: campaigns.reduce((sum: number, c: CampaignRow) => sum + c.clicks, 0),
        totalConversions: campaigns.reduce((sum: number, c: CampaignRow) => sum + c.conversions, 0),
      },
    });
  } catch (error) {
