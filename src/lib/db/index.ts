import { PrismaClient } from '@prisma/client';
import type { Prisma } from '@prisma/client';
import { NotFoundError } from '@/lib/errors';
import { redis } from '@/lib/queue/connection';
import { logger } from '@/lib/logger';

export const prisma = new PrismaClient();

/**
 * Campaign Database Queries
 */

export const campaignDb = {
  async create(data: Parameters<typeof prisma.adCampaign.create>[0]['data']) {
    return prisma.adCampaign.create({
      data,
      include: { creatives: true, dailyMetrics: true },
    });
  },

  async findById(id: string) {
    const campaign = await prisma.adCampaign.findUnique({
      where: { id },
      include: { creatives: true, dailyMetrics: { take: 30 } },
    });
    if (!campaign) throw new NotFoundError('Campaign', id);
    return campaign;
  },

  async findByOrganization(organizationId: string, skip = 0, take = 20) {
    return prisma.adCampaign.findMany({
      where: { organizationId },
      skip,
      take,
      orderBy: { createdAt: 'desc' },
      include: { creatives: true },
    });
  },

  async update(id: string, data: Parameters<typeof prisma.adCampaign.update>[0]['data']) {
    return prisma.adCampaign.update({
      where: { id },
      data,
      include: { creatives: true },
    });
  },

  async delete(id: string) {
    return prisma.adCampaign.update({
      where: { id },
      data: { deletedAt: new Date(), status: 'archived' },
      include: { creatives: true },
    });
  },

  async getMetrics(campaignId: string, days = 30) {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);

    return prisma.adMetricsDaily.findMany({
      where: {
        campaignId,
        date: { gte: startDate },
      },
      orderBy: { date: 'asc' },
    });
  },

  async getAggregatedMetrics(campaignId: string) {
    const metrics = await prisma.adMetricsDaily.findMany({
      where: { campaignId },
    });

    const totals = metrics.reduce(
      (acc, m) => ({
        impressions: acc.impressions + Number(m.impressions),
        clicks: acc.clicks + Number(m.clicks),
        conversions: acc.conversions + Number(m.conversions),
        spend: acc.spend + Number(m.spendCents),
      }),
      { impressions: 0, clicks: 0, conversions: 0, spend: 0 }
    );

    return {
      ...totals,
      ctr: totals.impressions > 0 ? (totals.clicks / totals.impressions * 100).toFixed(2) : 0,
      cpc: totals.clicks > 0 ? (totals.spend / totals.clicks / 100).toFixed(2) : 0,
      cpa: totals.conversions > 0 ? (totals.spend / totals.conversions / 100).toFixed(2) : 0,
    };
  },
};

export const creativeDb = {
  async create(data: Parameters<typeof prisma.adCreative.create>[0]['data']) {
    return prisma.adCreative.create({ data });
  },

  async findByCampaign(campaignId: string) {
    return prisma.adCreative.findMany({
      where: { campaignId, deletedAt: null },
      orderBy: { createdAt: 'desc' },
    });
  },

  async findById(id: string) {
    const creative = await prisma.adCreative.findUnique({ where: { id } });
    if (!creative || creative.deletedAt) throw new NotFoundError('AdCreative', id);
    return creative;
  },

  async update(id: string, data: Parameters<typeof prisma.adCreative.update>[0]['data']) {
    return prisma.adCreative.update({ where: { id }, data });
  },

  async delete(id: string) {
    return prisma.adCreative.update({ where: { id }, data: { deletedAt: new Date() } });
  },
};

/**
 * Lead Database Queries
 */

export const leadDb = {
  async capture(data: Parameters<typeof prisma.lead.create>[0]['data']) {
    return prisma.lead.create({ data });
  },

  async findById(id: string) {
    const lead = await prisma.lead.findUnique({ where: { id } });
    if (!lead) throw new NotFoundError('Lead', id);
    return lead;
  },

  async findByEmail(email: string) {
    return prisma.lead.findFirst({ where: { email } });
  },

  async findByOrganization(organizationId: string, skip = 0, take = 20) {
    return prisma.lead.findMany({
      where: { organizationId },
      skip,
      take,
      orderBy: { score: 'desc' },
    });
  },

  async update(id: string, data: Parameters<typeof prisma.lead.update>[0]['data']) {
    return prisma.lead.update({ where: { id }, data });
  },

  async getLeadsByScore(organizationId: string, minScore: number) {
    return prisma.lead.findMany({
      where: { organizationId, score: { gte: minScore } },
      orderBy: { score: 'desc' },
    });
  },

  async getConversionFunnel(organizationId: string, startDate: Date, endDate: Date) {
    const leads = await prisma.lead.findMany({
      where: {
        organizationId,
        createdAt: { gte: startDate, lte: endDate },
      },
    });

    return {
      total: leads.length,
      new: leads.filter(l => l.status === 'new').length,
      contacted: leads.filter(l => l.status === 'contacted').length,
      qualified: leads.filter(l => l.status === 'qualified').length,
      converted: leads.filter(l => l.status === 'converted').length,
    };
  },

  async updateScore(id: string, score: number, tier: string, priceCents?: number, explanation?: unknown) {
    const data: Record<string, unknown> = { score, tier };
    if (priceCents !== undefined) data.priceCents = priceCents;
    if (explanation) data.explanation = JSON.stringify(explanation);
    return prisma.lead.update({ where: { id }, data: data as Prisma.LeadUncheckedUpdateInput });
  },

  async appendFeedback(id: string, feedback: unknown) {
    const data: Record<string, unknown> = { feedbackJson: JSON.stringify(feedback) };
    return prisma.lead.update({ where: { id }, data: data as Prisma.LeadUncheckedUpdateInput });
  },
};

/**
 * Video Database Queries
 */

export const videoDb = {
  async create(data: Parameters<typeof prisma.videoAsset.create>[0]['data']) {
    return prisma.videoAsset.create({
      data,
      include: { variants: true },
    });
  },

  async findById(id: string) {
    const video = await prisma.videoAsset.findUnique({
      where: { id },
      include: { variants: true },
    });
    if (!video) throw new NotFoundError('Video', id);
    return video;
  },

  async update(id: string, data: Parameters<typeof prisma.videoAsset.update>[0]['data']) {
    return prisma.videoAsset.update({
      where: { id },
      data,
      include: { variants: true },
    });
  },

  async getVariants(videoId: string) {
    return prisma.videoVariant.findMany({
      where: { videoId },
      orderBy: { quality: 'desc' },
    });
  },

  async getProcessingQueue(limit = 10) {
    return prisma.videoProcessingQueue.findMany({
      where: { status: 'pending' },
      orderBy: { priority: 'desc' },
      take: limit,
    });
  },
};

/**
 * Feed Database Queries
 */

const invalidateFeedCache = async (userId?: string) => {
  if (!userId) return;
  try {
    const keys = await redis.keys(`feed:ranked:*:${userId}:*`);
    if (keys.length) {
      await redis.del(...keys);
    }
  } catch (error) {
    logger.warn('feed cache invalidation skipped', { error: (error as Error).message });
  }
};

const rewardForAction = (action: string, durationSeconds?: number | null) => {
  if (action === 'share') return 1.5;
  if (action === 'comment') return 1.2;
  if (action === 'like') return 1.0;
  if (action === 'view') {
    if (!durationSeconds) return 0.05;
    return Math.min(1, 0.05 + durationSeconds / 120); // cap modest reward for dwell
  }
  return 0;
};

export const feedDb = {
  async recordInteraction(data: Parameters<typeof prisma.userInteraction.create>[0]['data'] & { bucket?: string; experiment?: string }) {
    const { bucket: bucketMeta, experiment: experimentMeta, ...interactionData } = data;

    const interaction = await prisma.$transaction(async (tx) => {
      const interaction = await tx.userInteraction.create({ data: interactionData });

      const action = data.actionType;
      const contentId = data.contentId;
      const durationSeconds = data.durationSeconds ?? 0;

      try {
        if (action === 'like') {
          await tx.orgPost.update({ where: { id: contentId }, data: { likes: { increment: 1 } } });
        } else if (action === 'comment') {
          await tx.orgPost.update({ where: { id: contentId }, data: { comments: { increment: 1 } } });
        } else if (action === 'share') {
          await tx.orgPost.update({ where: { id: contentId }, data: { shares: { increment: 1 } } });
        } else if (action === 'view' && durationSeconds > 0) {
          await tx.orgPost.update({ where: { id: contentId }, data: { watchTime: { increment: BigInt(durationSeconds) } } });
        }
      } catch (err) {
        // If the post is missing, ignore to avoid failing the interaction write
        console.error('feedDb.recordInteraction post update failed', err);
      }
      return interaction;
    });

    await invalidateFeedCache(data.userId);

    // Log ranking feedback and bandit reward
    const reward = rewardForAction(data.actionType as string, data.durationSeconds ?? null);
    const bucket = bucketMeta;
    const experiment = experimentMeta ?? 'feed-ranking-v1';

    const prismaModels = prisma as unknown as Record<string, unknown>;

    (prismaModels['rankingFeedback'] as { create: (args: unknown) => Promise<unknown> })
      .create({
        data: {
          userId: data.userId,
          postId: data.contentId,
          bucket,
          experiment,
          action: data.actionType as string,
          reward,
        },
      })
      .catch((error: unknown) => logger.warn('rankingFeedback log skipped', { error: (error as Error).message }));

    (prismaModels['banditArm'] as { upsert: (args: unknown) => Promise<unknown> })
      .upsert({
        where: { contentId: data.contentId },
        create: {
          contentId: data.contentId,
          alpha: 1 + reward,
          beta: 1 + Math.max(0, 1 - reward),
          impressions: 1,
          clicks: reward > 0 ? 1 : 0,
        },
        update: {
          alpha: { increment: reward },
          beta: { increment: Math.max(0, 1 - reward) },
          impressions: { increment: 1 },
          clicks: { increment: reward > 0 ? 1 : 0 },
        },
      })
      .catch((error: unknown) => logger.warn('banditArm update skipped', { error: (error as Error).message }));

    return interaction;
  },

  async getUserInteractions(userId: string, days = 30) {
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);

    return prisma.userInteraction.findMany({
      where: { userId, timestamp: { gte: startDate } },
      orderBy: { timestamp: 'desc' },
    });
  },

  async getContentFeatures(contentId: string) {
    return prisma.contentFeature.findUnique({ where: { contentId } });
  },

  async getRecentPosts(limit = 20, offset = 0, filter: string = 'latest', type?: string, tag?: string) {
    const where: Record<string, unknown> = { visibility: 'public' };

    if (filter === 'media') {
      where.OR = [
        { mediaUrl: { not: null } },
        { type: { equals: 'media', mode: 'insensitive' } },
      ];
    } else if (filter === 'jobs') {
      where.OR = [
        { title: { contains: 'job', mode: 'insensitive' } },
        { content: { contains: 'job', mode: 'insensitive' } },
        { type: { equals: 'job', mode: 'insensitive' } },
      ];
    } else if (filter === 'money') {
      where.OR = [
        { title: { contains: 'money', mode: 'insensitive' } },
        { content: { contains: 'money', mode: 'insensitive' } },
        { content: { contains: 'finance', mode: 'insensitive' } },
        { type: { equals: 'money', mode: 'insensitive' } },
      ];
    } else if (filter === 'wellbeing') {
      where.OR = [
        { title: { contains: 'wellbeing', mode: 'insensitive' } },
        { content: { contains: 'wellbeing', mode: 'insensitive' } },
        { content: { contains: 'health', mode: 'insensitive' } },
        { type: { equals: 'wellbeing', mode: 'insensitive' } },
      ];
    }

    if (type) {
      where.type = { equals: type, mode: 'insensitive' };
    }

    if (tag) {
      where.tags = { has: tag };
    }

    return prisma.orgPost.findMany({
      where,
      skip: offset,
      take: limit,
      orderBy: { createdAt: 'desc' },
      include: { organization: true },
    });
  },
};

/**
 * Analytics Database Queries
 */

export const analyticsDb = {
  async getCampaignRevenue(organizationId: string, startDate: Date, endDate: Date) {
    const campaigns = await prisma.adCampaign.findMany({
      where: {
        organizationId,
        createdAt: { gte: startDate, lte: endDate },
      },
      include: { dailyMetrics: true },
    });

    return campaigns.map(c => ({
      id: c.id,
      name: c.name,
      spend: c.dailyMetrics.reduce((sum, m) => sum + Number(m.spendCents), 0),
      impressions: c.dailyMetrics.reduce((sum, m) => sum + Number(m.impressions), 0),
      clicks: c.dailyMetrics.reduce((sum, m) => sum + Number(m.clicks), 0),
      conversions: c.dailyMetrics.reduce((sum, m) => sum + Number(m.conversions), 0),
    }));
  },

  async getCreatorRevenue(creatorId: string, startDate: Date, endDate: Date) {
    const payouts = await prisma.creatorPayout.findMany({
      where: {
        creatorId,
        createdAt: { gte: startDate, lte: endDate },
      },
    });

    const gifts = await prisma.giftTransaction.findMany({
      where: {
        creatorId,
        createdAt: { gte: startDate, lte: endDate },
      },
    });

    const totalPayouts = payouts.reduce((sum, p) => sum + Number(p.totalAmount), 0);
    const totalGifts = gifts.reduce((sum, g) => sum + Number(g.creatorEarnings), 0);

    return {
      payouts: totalPayouts,
      gifts: totalGifts,
      total: totalPayouts + totalGifts,
      count: { payouts: payouts.length, gifts: gifts.length },
    };
  },

  async getTopCreators(limit = 10) {
    const creators = await prisma.giftTransaction.groupBy({
      by: ['creatorId'],
      _sum: { valueCents: true },
      orderBy: { _sum: { valueCents: 'desc' } },
      take: limit,
    });

    return creators;
  },
};

